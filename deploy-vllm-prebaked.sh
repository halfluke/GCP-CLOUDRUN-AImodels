...
# Default OFF: on Cloud Run L4 with newer host drivers (CUDA 13 / driver 580+),
# enabling cuda-compat causes Error 803 (lib conflict). Set to 1 only for older hosts.
...
DEPLOY_ARGS=(
  ...
  --port 8080
  --timeout "${TIMEOUT}"
  --set-env-vars "MAX_MODEL_LEN=${MAX_MODEL_LEN},DTYPE=${DTYPE},GPU_MEMORY_UTILIZATION=${GPU_MEMORY_UTILIZATION},VLLM_ENABLE_CUDA_COMPATIBILITY=${VLLM_ENABLE_CUDA_COMPATIBILITY},LD_LIBRARY_PATH=${LD_LIBRARY_PATH},TOOL_CALL_PARSER=${TOOL_CALL_PARSER},ENABLE_AUTO_TOOL_CHOICE=true"
)
